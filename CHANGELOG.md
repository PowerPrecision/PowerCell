...
### Corrigido
- **Botão "Portal do Cliente" transformado em DropdownMenu com "Ver como Cliente"** (`ux` — **UX**): Em `frontend/src/pages/ProcessDetails.js`, o antigo `<Popover>` "Portal do Cliente" (com Copiar Link / Enviar por Email) e o botão separado "Ver como Cliente" (impersonate) foram fundidos num único `<DropdownMenu>` (shadcn/ui). O trigger é um botão "Portal do Cliente ▾" (teal, com `ExternalLink` + `ChevronDown`). O menu suspenso contém: `Copiar Link` (gera magic link e copia), `Enviar por Email` (envia magic link por email), um separador, e `👁️ Ver como Cliente` (amber — chama `POST /api/portal/impersonate/{id}` e abre o Portal do Cliente num novo separador). Os handlers de 404 continuam a extrair o `detail` do backend (interceptor global silencioso em 404). Import de `DropdownMenu*` adicionado ao topo do ficheiro.
- **Cartão "Credenciais de Portais Oficiais" agora colapsa quando vazio** (`ux` — **UX**): Os cartões de Credenciais (1º e 2º proponente) em `ProcessDetails.js` não respeitavam a regra de auto-colapsar quando vazios. Adicionado o prop `collapsible` ao `CardHeaderWithEdit` de ambos os cartões (`financial_credenciais` e `financial_credenciais_2`), o corpo (`<p>` + grid de inputs) envolvido em `{!shouldCardBeCollapsed('...') && (<>...</>)}` (mantendo o cabeçalho sempre visível para expandir), e novos casos `financial_credenciais` / `financial_credenciais_2` na função `isCardEmpty` (verifica `portal_financas_utilizador`, `portal_financas_senha`, `seg_social_utilizador`, `seg_social_senha` — em `financialData` ou `titular2Data` conforme o proponente). Aplicada a mesma lógica de `isCollapsed` + isolamento de edição (`editingCardId`) dos cartões Financeiros.
- **Pesquisa na Lista de Clientes deixa de disparar ao escrever** (`ux` — **UX**): Em `frontend/src/pages/ClientsPage.js`, o `<Input>` de pesquisa estava ligado diretamente a `searchTerm` (URL param) via `onChange`, pelo que cada tecla disparava um `fetchClients`. Agora o input está envolvido num `<form onSubmit={...}>` com um botão "Pesquisar" (ícone `Search`). Adicionado estado local `searchInput` (inicializado de `searchTerm` e sincronizado via `useEffect` quando o URL muda). A query só é commitada ao URL (`setSearchTerm(searchInput)`) ao submeter o formulário (Enter ou clique no botão). Os filtros Select continuam a atualizar o URL imediatamente.

### Melhorado
- **Lógica de Fases em Clientes com Múltiplos Processos** (`feat` — **LÓGICA DE NEGÓCIO**): O conceito de "Fase do Cliente" era falacioso — um cliente pode ter 2 processos ativos em fases diferentes. Agora:
  - **Backend** (`backend/routes/clients.py`, `GET /api/clients`): ambos os code paths (`show_all` e não-`show_all`) constroem agora um array `active_processes: [{ref, status, status_label, status_color}]` por cliente, com um mini-objeto por processo ativo (`ref` = `process_number`). `fase_principal` mantém-se para retrocompatibilidade (fallback). A deteção de "ativo" reutiliza a condição já existente em cada path.
  - **Frontend** (`frontend/src/pages/ClientsPage.js`): a coluna "Fase" (vistas cartão + tabela) renderiza agora um badge por processo ativo no formato `[PROC-001: Triagem]` (com a cor do estado), em vez de um único badge. Se `active_processes` estiver vazio, faz fallback para `fase_principal` (e se não houver, mostra `-`).
  - **Pesquisa em Processos** (`backend/routes/processes.py`, `GET /api/processes`): a pesquisa textual (query param `search`) agora usa `build_multiword_search_filter(search, "client_name")` (multiword + sem acentos, mais robusta que `create_accent_insensitive_regex`) e faz `$match` também em `process_number`. Assim, ao pesquisar "Manuel" aparecem **todos** os processos desse cliente, mesmo que tenha vários processos ativos em fases diferentes.

### Notas
- **Backward compat**: `fase_principal` e `active_processes_count` continuam a ser devolvidos pelo `GET /api/clients` — o frontend usa `active_processes` se presente (com fallback para `fase_principal`). Outras páginas que consomem `fase_principal` (ex.: `ClientDetailPage`, `MyClientsPage`) não são afetadas.
- **DropdownMenu vs Popover**: o `Popover` continua importado em `ProcessDetails.js` (usado noutros sítios: datas, color picker). Apenas o bloco "Portal do Cliente" mudou para `DropdownMenu`.
- **Pesquisa em Processos**: a mudança de `create_accent_insensitive_regex` para `build_multiword_search_filter` só afeta o campo `client_name`. `client_email` e `process_number` continuam com regex simples (case-insensitive). O comportamento multiword ("Manuel Silva" → ambas as palavras têm de aparecer no nome) é o esperado e consistente com a pesquisa de clientes.

## [2026-06-19] — Pacote D: Afinações de Fluxo (Formulário, Logs Silenciosos e UX Automações)

### Adicionado
- **Criação automática de processo + email de convite no Formulário Público** (`feat` — **AUTOMAÇÃO**): O endpoint `POST /api/public/client-registration` em `backend/routes/public.py` agora cria AUTOMATICAMENTE um processo em `pre_registo` (anteriormente: não criava processo — triagem manual obrigatória) e envia o email de convite para o Portal do Cliente usando o email do sistema. Fluxo: (1) upsert do cliente (existente), (2) `get_next_process_number()` + `db.processes.insert_one` com `status="pre_registo"`, `is_active=True`, `is_deleted=False`, `fonte="public_form"`, (3) atualiza `client.process_ids`, (4) `create_client_magic_token(process_id)` + `secrets.token_urlsafe(6)[:8]` → `short_id` guardado em `portal_tokens` com `source="public_form_auto"`, (5) `send_email(account_name="power", to_emails=[email], subject="Bem-vindo ao seu Portal do Cliente — {nome}", force_system=True, system_purpose="NOTIFICATIONS")` com HTML body contendo o botão "Aceder ao meu Portal". O URL do portal é resolvido por Referer/Origin header → env var `FRONTEND_URL` (igual ao magic link normal). Se a criação do processo ou envio do email falhar, o registo do cliente NÃO falha (log warning + `magic_link_sent=False` na resposta) — o staff pode reenviar o link manualmente a partir do CRM. Resposta do endpoint agora inclui `process_id` e `magic_link_sent`.
- **Ação `create_task` no motor de automação** (`feat` — **UX**): Nova ação `create_task` adicionada a `VALID_ACTIONS` em `backend/services/workflow_engine.py` + handler em `execute_action` que cria uma tarefa em `db.tasks` com `title`, `urgency` (low/medium/high), `assigned_role` (resolve para o utilizador associado ao processo: consultor/mediador/indexador), `due_in_days` (opcional, calcula `due_date`), `source="automation"`, `rule_id`, `rule_name`. Substitui variáveis de contexto no título (`{client_name}`, `{status}`, `{process_id}`). Definida em `backend/routes/automation.py` com config_fields: `title` (text, default "Contactar {client_name}"), `urgency` (select: low/medium/high, default medium, com option_labels Baixa/Média/Alta), `assigned_role` (select: consultor/intermediario/mediador/indexacao, com option_labels), `due_in_days` (number, default 7).

### Corrigido
- **Indexador silencioso em documentos — barra de bloqueio explícita** (`fix` — **AUDITORIA**): Adicionado IF explícito em `backend/services/history.py::log_history` que bloqueia a criação de registo de histórico quando a ação é de carregamento/eliminação de documento (`action.startswith("Carregou documento")` ou `action.startswith("Eliminou documento")`) E o utilizador tem `role == "indexacao"`. Complementa o modo fantasma geral (que já silencia TODAS as ações do indexador) — a duplicação é intencional: se alguém remover o modo fantasma geral no futuro, o silêncio em documentos mantém-se. Adicionadas barras de bloqueio equivalentes (`if user and user.get("role") != "indexacao"`) nos 3 sítios de `backend/routes/documents.py` que fazem `db.history.insert_one` direto (linhas 4275, 4364, 4414) — estes BYPASSAM `log_history` e precisavam da mesma proteção. O indexador atua de forma totalmente silenciosa no mural do processo (documentos solicitados via portal, status de documentos alterado, pedidos de documento removidos).

### Melhorado
- **Construtor visual If/Then com Selects na página de Automações** (`ux` — **PACOTE D**): `frontend/src/pages/AutomationPage.js` agora renderiza Selects do shadcn/ui para os tipos de `config_fields` `select`, `select_status`, `select_role`, `select_user` (anteriormente: tudo era `<Input>` de texto, obrigando o utilizador a digitar o ID do estado/utilizador/role). Adicionado helper `renderConfigField(field, configKey)` que escolhe o controlo certo consoante `field.type`: `text/number` → `<Input>`, `textarea` → `<Textarea>`, `select` → `<Select>` com `field.options` + `field.option_labels`, `select_status` → `<Select>` populado por `GET /admin/workflow-statuses`, `select_role` → `<Select>` com `INTERNAL_ROLES` hardcoded (admin/ceo/diretor/consultor/intermediario/administrativo/indexacao), `select_user` → `<Select>` populado por `GET /users`. Novo `useEffect` faz `fetchSelectOptions()` para popular workflowStatuses e users. Exemplo concreto do pedido do user agora funciona: "Se o estado for [Select: status]" → "Criar tarefa: [Input: título], urgência: [Select: Baixa/Média/Alta], atribuída a [Select: Consultor/Intermediário/Mediador/Indexação]". **Nada de inputs de JSON em bruto** — o utilizador escolhe sempre de uma dropdown ou digita texto curto. `select_email_template` mantém-se como `<Input>` transitório (sem endpoint de templates ainda).

### Notas
- **Mudança de design no formulário público**: anteriormente o header de `public.py` dizia "NÃO se cria processo na submissão pública (triagem manual obrigatória)". A partir do Pacote D, o processo É criado em `pre_registo` automaticamente, e o cliente recebe o link do portal imediatamente por email. O staff continua a fazer triagem (mudar status, atribuir consultor) — o processo aparece no Kanban em `pre_registo`. Se for necessário reverter, basta remover o bloco "PACOTE D — CRIAÇÃO AUTOMÁTICA DE PROCESSO + EMAIL DE CONVITE" em `public.py`.
- **Indexador silencioso**: o IF em `history.py` é a "barra de bloqueio" pedida pelo user — mostra-se abaixo isoladamente. Os 3 sítios em `documents.py` com `db.history.insert_one` direto foram corrigidos porque NÃO passam por `log_history` (bypass do modo fantasma).
- **Automação `create_task`**: a tarefa é atribuída ao utilizador com o role indicado que JÁ ESTÁ associado ao processo (consultor/mediador/indexador). Se o processo não tiver utilizador com esse role, a tarefa fica sem atribuição (visível para admins). Isto é intencional — a automação não cria novas atribuições, apenas tarefas para as atribuições existentes.
...
### Corrigido
- **"Erro ao enviar email" silencioso ao clicar em "Enviar por Email" do Portal do Cliente em processos eliminados** (`fix` — **UX/DEBUG**): O `POST /api/processes/{id}/generate-magic-link[/send]` filtrava `is_deleted: {"$ne": True}` na query MongoDB, pelo que devolvia 404 genérico "Processo não encontrado" sempre que o processo estivesse soft-deleted — sem distinção entre "realmente não existe" e "foi eliminado". Como o `GET /api/processes/{id}` (que abre a página de Detalhes do Processo) **não** filtra `is_deleted`, o utilizador ficava com a página aberta, clicava no botão e recebia um 404 silencioso (o interceptor global do `api.js` é silencioso para 404, só faz `console.warn`), com o toast local a mostrar apenas "Erro ao enviar email". Agora os dois endpoints (`generate_magic_link` e `send_magic_link_email`) fazem lookup **sem** o filtro `is_deleted` e devolvem **404 com mensagem acionável** quando o processo está eliminado: "Este processo foi eliminado. Restaure-o para gerar o magic link." (ou "...para enviar o magic link por email."). `backend/routes/processes.py`.
- **Toast genérico "Erro ao enviar email" / "Erro ao gerar link" não mostrava a causa real** (`fix` — **UX**): O bloco `catch` dos botões "Copiar Link" e "Enviar por Email" do popover do Portal do Cliente em `ProcessDetails.js` mostrava sempre um toast genérico, ignorando o `detail` devolvido pelo backend. Como o interceptor global do `api.js` é silencioso em 404 (apenas `console.warn`), o utilizador nunca via a causa real. Agora o `catch` extrai `error.response.data.detail` quando `status === 404` e mostra-o num toast (ex.: "Este processo foi eliminado. Restaure-o para gerar o magic link."). Outros status (400 sem email, 500 falha de envio, etc.) continuam a ser tratados pelo interceptor global — sem duplicação de toasts. `frontend/src/pages/ProcessDetails.js`.

### Notas
- **Cenário do user**: processo `71448d96-7efd-47f8-a199-9cbdc6d105b6` retornava 404 no `POST /api/processes/{id}/generate-magic-link/send`. Verificado via `curl` que a rota **existe** no deploy (`GET` devolve 405 Method Not Allowed, `POST` sem auth devolve 401) — o 404 vinha de dentro do handler, do `if not process: raise HTTPException(404)`. Como o `GET /processes/{id}` (que não filtra `is_deleted`) deixava abrir a página, a causa era o processo estar `is_deleted: True` (script de seed massivo cria 5% eliminados; ou eliminação manual).
- **Segurança**: o Portal do Cliente continua a recusar acessos a processos eliminados — `backend/routes/portal.py` filtra `is_deleted: {"$ne": True}` em todos os endpoints que o cliente usa (linhas 258, 628, 705). Ou seja, mesmo que um magic link fosse gerado para um processo eliminado, o cliente não conseguia aceder ao portal. Por isso faz mais sentido **bloquear a geração** com mensagem clara do que permitir gerar um link inútil.
- **Consistência**: o comportamento dos endpoints de magic link fica alinhado com o do `GET /processes/{id}` no que diz respeito a encontrar o processo (ambos procuram sem filtro `is_deleted`); a diferença é que os endpoints de magic link **recusam operar** sobre eliminados (com mensagem clara), enquanto o GET permite visualizá-los (para um admin os poder restaurar).
