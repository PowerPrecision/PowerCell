...
## Correções 2026-02 (Fev) — Fix: Regressões críticas de Onboarding (4 bugs)

- **Bug 1 (Processo criado indevidamente)**: `CreateClientModal.jsx` ganhou prop `clientOnly` — quando usado pelos botões "Novo Cliente" (`ClientsPage.js`, `MyClientsPage.js`), cria APENAS o Cliente/Lead (sem Processo, sem envio ao Índice). O botão "Novo Processo" do Kanban continua a criar cliente+processo (fluxo antigo preservado, `clientOnly` default `false`).
- **Bug 2 (Documentos hardcoded no Portal)**: 3 pontos hardcoded corrigidos — `process_create.py::create_default_portal_documents` (usava `DEFAULT_PENDING_CATEGORIES`, 4 categorias fixas → agora delega em `generate_mandatory_document_requests`, dinâmico do SystemConfig); `client_crud.py::run_create_client` passou a gerar os pedidos de documento na criação do cliente (antes não gerava nenhum); `portal_status.py` — fallback de docs pendentes lê SystemConfig dinamicamente (antes usava a mesma lista estática) e o `display_label` passou a preferir sempre `custom_label` (antes só para categoria "Outros", mostrando slugs crus como "identificacao" para as novas categorias dinâmicas). `DEFAULT_PENDING_CATEGORIES` removida por completo.
- **Bug 3 (Falha silenciosa de email)**: `portal_magic_link.py::send_magic_link_to_client` agora verifica o resultado de `send_email` e levanta `HTTPException(500)` com a razão real se falhar (antes ignorava o resultado e reportava sempre sucesso). `email.py::send_registration_confirmation` (email de boas-vindas) passou a usar `email_service.py::send_email` (ligado ao SystemConfig/Resend) em vez de `email_v2.py` (dependia de variáveis de ambiente nunca configuradas, caindo sempre em modo simulado); `client_portal_email.py` agora loga corretamente sucesso/falha real.
