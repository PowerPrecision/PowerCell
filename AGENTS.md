...
- **Testes unitários do frontend: `yarn test`, bloqueantes no CI.** `node --test` sobre `src/**/*.test.js(x)`, enumerados por `scripts/run-unit-tests.mjs` (os globs do `node --test` só existem no Node 22; o CI corre Node 20). Assertivas em `node:assert/strict` com `describe`/`it` de `node:test` — **não há Jest nem Vitest instalados**, e três ficheiros escritos com `expect(...)` nunca correram até serem convertidos (Set 2026). Imports nos testes levam extensão explícita (`./x.js`).
