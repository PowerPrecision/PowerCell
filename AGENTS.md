...
- CI (`.github/workflows/main.yml`) builds only `frontend/` (Vite build + ESLint) and `backend/` (flake8 + pytest against a `mongo:7` service) plus a security scan (bandit + pip-audit).
- **Frontend E2E (Playwright)**: not run in CI. To run locally against the dev stack: `cd frontend && npx playwright install chromium` (browsers aren't pre-installed), then `PLAYWRIGHT_BASE_URL=http://localhost:3000 PLAYWRIGHT_API_URL=http://localhost:8001/api npx playwright test e2e/<spec> --project=chromium`. Specs that need data (e.g. `e2e/undo-delete.spec.js`) provision it via the API and clean up after.
