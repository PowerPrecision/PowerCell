...
async def create_activity(data: ActivityCreate, user: dict = Depends(get_current_user)):
    ...
    # ================================================================
    # PACOTE BJ — STEALTH MODE (create_activity)
    # ================================================================
    # A rota cria_activity insere DIRETAMENTE em db.activities (linha
    # abaixo) antes de chamar log_history(). Como esta inserção direta
    # não passa pelo guard do log_history, precisamos de uma barra de
    # bloqueio explícita aqui para manter a consistência do stealth
    # mode: utilizadores "indexacao" ou com track_history=False não
    # deixam comentários visíveis no mural do processo.
    # ---------------------------------------------------------------
    # Decisão de UX: um utilizador silenciado que tenta adicionar um
    # comentário recebe 403 com mensagem clara em vez de um erro
    # silencioso (que o confundiria). O comentário é bloqueado porque
    # seria contraditório silenciar o log_history("Adicionou comentário")
    # mas deixar o comentário visível na coleção activities.
    # ================================================================
