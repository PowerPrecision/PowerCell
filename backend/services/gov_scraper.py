...
- Espera explícita pelo DataTables (JavaScript) que renderiza as
  tabelas de comprovativos no Portal das Finanças. Sem esta espera,
  os locators falham e o scraper cai no fallback page.pdf(), que
  guarda o HTML da página em vez do PDF.
- Download nativo via expect_download: o Playwright interceta o
  evento de download do browser, obtendo os bytes reais do ficheiro.
  Validação rigorosa via magic bytes (%PDF-) garante que nunca
  guardamos HTML disfarçado de PDF.
- Fallback _intercept_pdf_from_element (href + expect_response)
  caso o download nativo não dispare.
...
async def _financas_scraper_inner(nif: str, password: str, process_id: Optional[str] = None) -> ScraperResult:
    ...
    try:
        ...
        #
        # Estratégia (v5 — download nativo):
        # 1. Esperar pela tabela DataTables (wait_for_selector com células)
        # 2. Localizar botões de PDF na tabela (locators específicos)
        # 3. Interceptar o download nativo do browser (expect_download)
        # 4. Ler os bytes binários do ficheiro temporário
        # 5. Validar magic bytes %PDF- (nunca guardar HTML)
        # 6. Fallback: _intercept_pdf_from_element (href + expect_response)
        # 7. Último recurso: _download_financas_document genérico
        step = "download_irs"
        ...
        try:
            ...
            # que as CÉLULAS estejam visíveis antes de procurar os botões.
            # Usamos 'td' em vez de 'tr' para garantir que o conteúdo real
            # foi renderizado (DataTables pode criar <tr> vazios inicialmente).
            logger.info(
            ...
            try:
                ...
                alt_table_selectors = [
                    ...
                    "#comprovativos tbody tr td",
                    "table#listaDocumentos tbody tr td",
                ]
            ...
            # Estratégia prioritária: expect_download (download nativo do browser).
            # Se o portal não disparar download (abre inline), recorremos ao
            # fallback _intercept_pdf_from_element (href + expect_response).
            try:
                irs_locators = [
                    ...
                    # Qualquer link na 1ª linha da tabela (o portal pode usar
                    # texto simples ou ícone sem title específico)
                    'table#documentos tbody tr:first-child a',
                ...
                for selector in irs_locators:
                    try:
                        ...
                        if await irs_btn.count() > 0 and await irs_btn.is_visible(timeout=3000):
                            ...
                            # ── Tentativa 1: Download nativo do browser ──
                            # O Playwright interceta o evento de download e guarda
                            # o ficheiro num path temporário. Lemos os bytes reais
                            # do ficheiro (nunca do DOM).
                            try:
                                async with page.expect_download(timeout=30000) as download_info:
                                    await irs_btn.click(force=True)

                                download = await download_info.value
                                tmp_path = await download.path()

                                # Ler os bytes binários reais do ficheiro
                                # Usamos leitura síncrona dentro de asyncio
                                # (ficheiro local temporário, sem I/O de rede)
                                if tmp_path and tmp_path.exists():
                                    with open(tmp_path, "rb") as f:
                                        pdf_bytes = f.read()
                                    logger.info(
                                        f"[GOV_SCRAPER] Download nativo IRS obtido: "
                                        f"{len(pdf_bytes)} bytes de {download.suggested_filename}"
                                    )
                                else:
                                    logger.warning(
                                        "[GOV_SCRAPER] Download nativo IRS: "
                                        "ficheiro temporário não encontrado"
                                    )
                            except Exception as dl_err:
                                logger.warning(
                                    f"[GOV_SCRAPER] expect_download falhou para IRS "
                                    f"({selector}): {type(dl_err).__name__}: {dl_err}"
                                )
                            ...
                            if not pdf_bytes or not pdf_bytes.startswith(b"%PDF-"):
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"[GOV_SCRAPER] _intercept_pdf_from_element "
                                        f"falhou para IRS: {type(fallback_err).__name__}"
                                    )
            ...
            # ── 8. Extrair Nota de Liquidação (página de Consulta) ──
            # A Nota de Liquidação (Demonstração de Liquidação) NÃO está na
            # página de comprovativos (/comprovativo/obterComprovativo.action).
            # Fica na página de Consulta do IRS, onde é preciso navegar,
            # carregar a tabela do ano e expandir o detalhe.
            step = "download_nota"
            try:
                # 8a. Navegar para a página de Consulta onde residem as
                # Notas de Liquidação
                consulta_url = (
                    "https://irs.portaldasfinancas.gov.pt"
                    "/consulta/index.action"
                )
                logger.info(
                    f"[GOV_SCRAPER] A navegar para página de Consulta IRS: "
                    f"{consulta_url}"
                )
                ...
                liq_selectors = [
                    'a:has-text("Demonstração de Liquidação")',
                    'a:has-text("Nota de Liquidação")',
                    'a:has-text("Demonstracao de Liquidacao")',
                    'a:has-text("Nota de Liquidacao")',
                    'a[title*="Liquida"]',
                    'a[href*="liquidacao"]',
                    'a[href*="notaLiquidacao"]',
                    'button:has-text("Liquidação")',
                    'button:has-text("Liquidacao")',
                ]

                liq_btn = None
                for liq_sel in liq_selectors:
                    try:
                        candidate = page.locator(liq_sel).first
                        if await candidate.count() > 0:
                            liq_btn = candidate
                            logger.info(
                                f"[GOV_SCRAPER] Botão Nota de Liquidação "
                                f"encontrado via selector: {liq_sel}"
                            )
                            break
                    except Exception:
                        continue

                if liq_btn and await liq_btn.count() > 0:
                    pdf_bytes = None

                    # ── Tentativa 1: Download nativo do browser ──
                    try:
                        async with page.expect_download(timeout=30000) as download_info:
                            await liq_btn.click(force=True)
                        ...
                        if tmp_path and tmp_path.exists():
                            with open(tmp_path, "rb") as f:
                                pdf_bytes = f.read()
                            logger.info(
                                f"[GOV_SCRAPER] Download nativo Nota Liquidação "
                                f"obtido: {len(pdf_bytes)} bytes de "
                        ...
                        logger.warning(
                            ...
                            f"Nota Liquidação: {type(dl_err).__name__}: {dl_err}"
                        )
                    ...
                    if not pdf_bytes or not pdf_bytes.startswith(b"%PDF-"):
                        logger.info(
                            ...
                            "_intercept_pdf_from_element"
                        )
                        try:
                            pdf_bytes = await _intercept_pdf_from_element(
                                page, liq_btn, "Nota_Liquidacao_IRS"
                            )
                        except Exception as fallback_err:
                            logger.warning(
                                f"[GOV_SCRAPER] _intercept_pdf_from_element "
                                f"falhou para Nota Liquidação: "
                    ...
                    logger.warning(
                        "[GOV_SCRAPER] Botão Nota de Liquidação não encontrado "
                        "na página de Consulta"
                    )

                if not nota_downloaded:
                    logger.warning(
                        "[GOV_SCRAPER] Nota de Liquidação não extraída da "
                        "página de Consulta — a tentar extração genérica"
                    )
            except Exception as nota_err:
                logger.warning(
                    f"[GOV_SCRAPER] Erro na extração da Nota Liquidação "
                    f"(página de Consulta): {type(nota_err).__name__}: {nota_err}"
                )
            ...
            if still_on_auth:
                ...
                if still_on_auth_2:
                    ...
                    if mfa_detected and process_id:
                        ...
                        logger.info(
                            f"[GOV_SCRAPER] A preencher código MFA ({len(mfa_code)} "
                            f"dígitos) na Seg. Social para processo {process_id}"
                        )
                        try:
                            ...
                            for inp_sel in mfa_input_selectors:
                                try:
                                    ...
                                    if await inp_el.is_visible(timeout=2000):
                                        mfa_input = inp_el
                                        logger.info(
                                            f"[GOV_SCRAPER] Input MFA encontrado "
                                            f"via: {inp_sel}"
                                        )
                                        break
                                except Exception:
                            ...
                            if not mfa_input:
                                # Fallback: tentar qualquer input visível na página
                                logger.warning(
                                    "[GOV_SCRAPER] Input MFA específico não encontrado "
                                    "— a tentar input genérico"
                                )
                                all_inputs = page.locator("input[type='text'], input[type='tel'], input[type='number']").all()
                                for inp in all_inputs:
                                    try:
                            ...
                            if mfa_input:
                                # Limpar campo e preencher código
                                await mfa_input.click()
                                await mfa_input.fill("")
                                await mfa_input.type(mfa_code, delay=80)
                                logger.info(
                                    f"[GOV_SCRAPER] Código MFA preenchido "
                                ...
                                for btn_sel in mfa_submit_selectors:
                                    try:
                                        ...
                                        if await btn_el.is_visible(timeout=2000):
                                            ...
                                            submitted = True
                                            logger.info(
                                                f"[GOV_SCRAPER] Botão MFA clicado: {btn_sel}"
                                            )
                                            break
                                    except Exception:
                                        continue
                                ...
                                if not submitted:
                                    ...
                                    await mfa_input.press("Enter")
                                    logger.info(
                                        "[GOV_SCRAPER] MFA submetido via Enter"
                                    )

                                # Aguardar navegação após submissão MFA
                                try:
                                    await page.wait_for_load_state(
                                ...
                                # Limpar código MFA do Redis (segurança)
                                await delete_mfa_code(process_id)
                                ...
                                if still_on_mfa:
                                    ...
                                    logger.warning(
                                        ...
                                        f"de login — código pode estar incorreto"
                                    )
                                    screenshot_b64 = await _take_screenshot(page, "mfa_incorrect")
                                    return ScraperResult(
                                        success=False,
                                ...
                                logger.info(
                                    f"[GOV_SCRAPER] MFA aceito com sucesso para "
                                ...
                                logger.error(
                                    "[GOV_SCRAPER] Nenhum input MFA encontrado na página"
                                )
                                screenshot_b64 = await _take_screenshot(page, "mfa_no_input")
                                return ScraperResult(
                                    success=False,
                            ...
                            logger.error(
                                f"[GOV_SCRAPER] Erro ao preencher MFA: "
                                f"{type(mfa_err).__name__}: {mfa_err}"
                            )
                            screenshot_b64 = await _take_screenshot(page, "mfa_fill_error")
                            return ScraperResult(
                                success=False,
                        ...
                        # MFA detetado mas sem process_id — não podemos esperar
                        logger.warning(
                            "[GOV_SCRAPER] MFA detetado mas sem process_id — "
                            "não é possível aguardar código"
                        )
                        screenshot_b64 = await _take_screenshot(page, "mfa_no_process_id")
                        return ScraperResult(
                            success=False,
