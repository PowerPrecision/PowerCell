...
LAST_UCR_DELETE_DETAIL = (
    "Não é possível eliminar o único acesso do utilizador. "
    "Adicione outro primeiro."
)


def serialize_ucr(doc: dict, company_names: Optional[dict] = None) -> dict:
    """Normaliza um documento UCR para o frontend (id, company_name, role)."""
    if not doc:
        return doc
    out = dict(doc)
    if not out.get("id"):
    ...
    if not company_name and isinstance(out.get("company"), dict):
        company_name = out["company"].get("name") or out["company"].get(
            "company_name"
        )
    if not company_name and company_id and company_names:
        company_name = company_names.get(company_id)

    role = out.get("role") or out.get("role_name") or ""
...
async def run_delete_user_company_role(
    ...
    Impede apagar o último/único acesso do utilizador (400) para não
    deixar a conta sem UCR.
    """
    ...
    deleted = await db.user_company_roles.delete_one({"id": existing.get("id")})
