...
==============================================
Sincronização bidirecional entre PowerCell e Trello.
...
class TrelloService:
    ...
    async def create_webhook(self, callback_url: str, id_model: str = None, 
                             description: str = "PowerCell Sync") -> Dict:
...
async def init_trello_from_config():
    """
    ...
    try:
        ...
        if config.trello and config.trello.enabled:
            # Usar configurações da base de dados se disponíveis, senão usar env vars
            api_key = config.trello.api_key or TRELLO_API_KEY
            token = config.trello.api_token or TRELLO_TOKEN
            board_id = config.trello.board_id or TRELLO_BOARD_ID
            
            if api_key and token and board_id:
                trello_service.api_key = api_key
                trello_service.token = token
                trello_service.board_id = board_id
                logger.info(f"Serviço Trello inicializado com configuração da BD - Board: {board_id[:8]}...")
            else:
                logger.warning("Configuração Trello incompleta na BD")
        else:
            logger.info("Trello desativado nas configurações do sistema")
            
    except Exception as e:
        logger.warning(f"Não foi possível carregar configuração Trello da BD: {e}")
        # Continua com variáveis de ambiente se falhar
