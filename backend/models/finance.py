...
class FinanceConfig(BaseModel):
    ...
    @classmethod
    def coerce_default_value(cls, v):
    ...
    def validate_default_value_range(self):
        """
        Garantir que o default_value está dentro dos limites:
        - Se fee_type='percentage': não pode ultrapassar 100%
        - Se fee_type='fixed': valor em euros, sem limite superior
        """
        if self.fee_type == FeeType.PERCENTAGE.value and self.default_value > 100:
            raise ValueError(
                "Com percentagem, default_value não pode ultrapassar 100 "
                f"(recebido: {self.default_value})"
            )
        return self
...
class FinanceConfigUpdate(BaseModel):
    ...
    def coerce_default_value(cls, v):
        if v is None or v == '':
            return None
        return _coerce_to_non_negative_float(v)
...
class ProcessFinance(BaseModel):
    ...
    @classmethod
    def coerce_applied_fee_value(cls, v):
        """Coerção para float, não-negativo."""
        result = _coerce_to_non_negative_float(v)
        if result is None:
    ...
    @model_validator(mode='after')
    def validate_applied_fee_value_range(self):
        """
        Garantir que applied_fee_value está dentro dos limites:
        - Se applied_fee_type='percentage': não pode ultrapassar 100%
        - Se applied_fee_type='fixed': valor em euros, sem limite superior
        """
        if self.applied_fee_type == FeeType.PERCENTAGE.value and self.applied_fee_value > 100:
            raise ValueError(
                f"Com percentagem, applied_fee_value não pode ultrapassar 100 "
                f"(recebido: {self.applied_fee_value})"
            )
        return self
...
class ProcessFinanceCreate(BaseModel):
    ...
    explicitamente OU calculados automaticamente a partir de:
      base_business_value + applied_fee_type + applied_fee_value + tax_rate
    """
    process_id: str = Field(..., description="Referência ao ProcessModel")
    client_id: str = Field(..., description="Referência ao ClientModel")
    ...
    @classmethod
    def coerce_non_negative_floats(cls, v):
        result = _coerce_to_non_negative_float(v)
        if result is None:
    ...
    @model_validator(mode='after')
    def auto_calculate_or_validate(self):
        """
        Calcular automaticamente expected_commission, tax_amount e total_with_tax
        se não fornecidos explicitamente. Validar se fornecidos.
        """
        # Calcular expected_commission
        if self.expected_commission is None:
            if self.applied_fee_type == FeeType.PERCENTAGE.value:
                self.expected_commission = round(
                    self.base_business_value * (self.applied_fee_value / 100), 2
                )
            else:  # fixed
                self.expected_commission = self.applied_fee_value

        # Validar percentagem ≤ 100
        if self.applied_fee_type == FeeType.PERCENTAGE.value and self.applied_fee_value > 100:
            raise ValueError(
                f"Com percentagem, applied_fee_value não pode ultrapassar 100 "
                f"(recebido: {self.applied_fee_value})"
        ...
        if self.tax_amount is None:
            self.tax_amount = round(self.expected_commission * (self.tax_rate / 100), 2)

        # Calcular total_with_tax
        if self.total_with_tax is None:
            self.total_with_tax = round(self.expected_commission + self.tax_amount, 2)
